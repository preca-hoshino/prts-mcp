...
```bash
# 执行全量检查（包含格式化、Lint、类型推导、未使用的依赖、Jest 测试）
npm run check
```
...
1. **用户可见影响 (User-visible impact)**：清晰说明该修改对 API 接口表现、响应格式、系统配置或网关行为产生了什么影响。
2. **技术重构与取舍说明 (Architecture & Tradeoffs)**：如果是针对服务逻辑、中间件实现、数据库架构变动或外部依赖替换，请说明采用该方案的背景及背后的取舍逻辑。
3. **验证步骤 (Validation steps)**：列出如何通过 API 测试、自动化测试脚本或集成测试来验证本次修改，并确保 `npm run check` 自动化检查全部通过。
4. **测试日志/截图**：涉及到新的 API、逻辑调整或性能改进时，请提供终端测试结果输出、日志片段或 Benchmark 对比情况以供审核。
